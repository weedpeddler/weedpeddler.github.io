...
var GIST_ID = '9f4205315d30372bbe606c0363de024a'; // <-- Paste your Gist ID inside the quotes
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  } else if (GIST_ID && !cfg.gistId) {
    saveConfig(GIST_ID, cfg.tokenEnc || '');
  }
...
function handleUnlock() {
  ...
  if (!raw) {
    ...
    if (state.gistId) {
      syncMsg('Verifying...');
      var fetchP = state.tokenEnc ? dec(state.tokenEnc, code).then(function(t) { return gistFetch(t, state.gistId); }) : gistFetchPublic(state.gistId);
      fetchP.then(function(r) {
        if (r.tokenEnc && !state.tokenEnc) { state.tokenEnc = r.tokenEnc; saveConfig(state.gistId, r.tokenEnc); }
        if (r.data && r.data.length > 0) {
          setBlob(r.data);
          return dec(r.data[0], code).then(function(txt) {
  ...
  if (pos.length > 0) {
    ...
    state.filteredNotes.sort(function(a, b) {
      ...
      pos.forEach(function(w) {
        var re = w.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        var ca = (ta.match(new RegExp(re, 'g')) || []).length;
        var cb = (tb.match(new RegExp(re, 'g')) || []).length;
        sa += ca; sb += cb;
